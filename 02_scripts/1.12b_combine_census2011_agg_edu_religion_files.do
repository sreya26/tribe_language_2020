...
project , original("../01_data/census_2011_agg_edu_data/DDW-0000C-08SC.xlsx")	
project , original("../01_data/census_2011_agg_edu_data/DDW-0000C-08ST.xlsx")	
project , original("../01_data/census_2011_st_edu_data/C-9/DDW-0000C-09.xlsx")	
...
*************************************************************************************************************************
	...
	keep if tru == 1
	keep state_code state caste_group *_p
...
*************************************************************************************************************************
	...
	foreach x in p m f {
		foreach y of local upper_ages {
			tempvar age0to`y'_`x'
			egen `age0to`y'_`x'' = sum(total_`x') if inrange(upper_age,6,`y'), by(state_code religion tru)
			egen age0to`y'_`x' = max(`age0to`y'_`x'') , by(state_code religion tru)
			drop `age0to`y'_`x''
		}	
	}
	
	foreach y in p m f {
		egen matricplus_p_2011_`y' = rowtotal(matric_`y' intermediate_`y' nontechdiploma_`y' techdiploma_`y' graduate_`y')
	...
	drop tru
	clonevar tru = `tru'
	order religion tru, after(state)
...
project , creates("../08_temp/census2011_religion_edu_data.dta")	
