...
*********************************************************************************************************
* this file differs from 3.02_subsi_descriptive_stats_variant.do in one respect:
* the tables are made only for protected areas
********************************************************************************************************


global ifprotected "if scheduledarea6113~=0 | tribalarea6113~=0"
global mergeprotected `"merge m:1 region6113 using "../03_processed/scheduled_and_tribal_regions", keepusing(scheduledarea6113 tribalarea6113) assert(match using) keep(match) nogen"'
...
* -- amended to show three measures of linguistic distance
********************************************************************************************************
	...
	* -- amended to obtain three measures of linguistic distance
	********************************************************************************************************
	...
	collapse (mean) lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant [w=total_speakers] , by(castegroup_1961_2011_code)
	
*	rename lang_distance_f8_dominant linguistic_distance
	format lang_distance_??_dominant %4.2f
	...
	********************************************************************************************************
	...
	#delimit ;
	listtab castegroup_1961_2011_code india_tribe_perc l1 s1 
			lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant 
		...
		using "../04_results/01_tables/3_new_summary_table_2a_protected.tex", replace
		rstyle(tabular)
		head(
		`"\toprule"'
		`"& & & & & & 										& \multicolumn{2}{c}{\% that speaks} \\"'
		`"& & & & \multicolumn{3}{c}{Linguistic distance} 	& \multicolumn{2}{c}{dominant language as} & \multicolumn{5}{c}{Educational attainment in 2011, (\%) } \\"' 
		`"\cmidrule(lr){5-7} \cmidrule(lr){8-9} \cmidrule(lr){10-14}"'
		`"		& \% of 	& Largest 			& Largest		& Fearon 	& 		& Counting	& Mother 	& Subsidiary \\"'
		`"Tribe	& India ST 	& Mother Tongue 	& 1961 State 	& {m = 8} 	& ASJP  & nodes		& Tongue	& Language & Literacy & Primary & Middle & Secondary & Graduate\\"'
		`"\midrule"')
		foot(`"\bottomrule"')
		;
	#delimit cr
...
********************************************************************************************************
	...
	* -- amended to get three types of linguistic distance
	********************************************************************************************************
	...
	preserve
		collapse (mean) lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant [w=total_speakers]
		gen state_1961 = "India"
		tempfile india
		save `india'
	restore
	
	collapse (mean) lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant [w=total_speakers] , by(state_1961)
	...
	* -- amended to allow three types of linguistic distance
	********************************************************************************************************
	...
	merge 1:1 state_1961 using `outcomes', assert(match using) keep(match) nogen
	...
	keep state_1961 tribal_pop_perc dominant_language lang_distance_??_dominant mt_speakers_dom_lang_perc subsi_speakers_dom_lang_perc matricplus_*
	...
	#delimit ;
	listtab state_1961 tribal_pop_perc dominant_language 
			lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant 
			mt_speakers_dom_lang_perc subsi_speakers_dom_lang_perc
			matricplus_sc matricplus_st matricplus_nonscst
		using "../04_results/01_tables/3_new_summary_table_1a_protected.tex", replace
		rstyle(tabular)
		head(
		`"\toprule"'
		`"& & & \multicolumn{3}{c}{Linguistic distance} & \multicolumn{2}{c}{\% of tribal population} & \multicolumn{3}{c}{Secondary school}\\"'
		`"					& 		& 			& \multicolumn{3}{c}{among STs} & \multicolumn{2}{c}{that speaks} & \multicolumn{3}{c}{attainment in} \\"'
		`"					& 		& 			& & & 							& \multicolumn{2}{c}{dominant language as} & \multicolumn{3}{c}{2011 (\%)}\\"'
		`"										\cmidrule(lr){4-6}			\cmidrule(lr){7-8} 								\cmidrule(lr){9-11}"'
		`"					& 		& Dominant 	& Fearon	& 		& Counting 	& Mother 	& Subsidiary &		&		& Non-\\"'
		`"Census 1961 State & ST \%	& Language	& m = 8		& ASJP	& nodes		& Tongue	& Language 	 & SC	& ST 	& SC/ST \\"'
		`"\midrule"')
		foot(`"\bottomrule"')
		;
	#delimit cr
...
********************************************************************************************************
	...
	* -- amended to compute three measures of linguistic distance
	********************************************************************************************************
	...
	collapse (mean) lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant [w=total_speakers] , by(state_1961 castegroup_1961_2011_code)
...
*	rename lang_distance_f8_dominant linguistic_distance
	...
	* -- amended to allow three measures of linguistic distance
	********************************************************************************************************
	...
	order state_1961 castegroup_1961_2011_code pop_st_perc lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant ///
			mt_speakers_dom_lang_perc subsi_speakers_dom_lang_perc matricplus_perc_2011
	
	reshape wide castegroup_1961_2011_code pop_st_perc lang_distance_f8_dominant lang_distance_as_dominant lang_distance_tj_dominant ///
			mt_speakers_dom_lang_perc subsi_speakers_dom_lang_perc matricplus_perc_2011 , i(state_1961) j(state_rank)
	...
	* drop small states ( < 5 million in census 2011) and those with a small ST population ( < 1% of state population)
	...
	drop *4  // we construct the table only for the top 3 tribes and drop the fourth
	
	#delimit ;
	listtab 
		using "../04_results/01_tables/3_new_summary_table_3a_protected.tex", replace
		rstyle(tabular)
		head(
		`"\toprule"'
		`"& & & \multicolumn{3}{c}{Linguistic} & \multicolumn{2}{c}{\% that speaks}		 & & & &  \multicolumn{3}{c}{Linguistic} & \multicolumn{2}{c}{\% that speaks} 		& & & &  \multicolumn{3}{c}{Linguistic} & \multicolumn{2}{c}{\% that speaks} & \\"'
		`"& & &  \multicolumn{3}{c}{distance} & \multicolumn{2}{c}{dominant language as}   & & & & \multicolumn{3}{c}{distance} & \multicolumn{2}{c}{dominant language as} & & & & \multicolumn{3}{c}{distance} & \multicolumn{2}{c}{dominant language as} & \\"' 
		`"\cmidrule(lr){4-6} \cmidrule(lr){7-8} \cmidrule(lr){12-14} \cmidrule(lr){15-16} \cmidrule(lr){20-22} \cmidrule(lr){23-24} "'
		`"				& Largest   & \% of 	& Fearon & & Counting 	& Mother 	& Subsi. & Secondary     & 2nd Largest   & \% of 	& Fearon & & Counting 	& Mother 	& Subsi. & Secondary     & 3rd Largest   & \% of 	& Fearon & & Counting 	& Mother 	& Subsi. & Secondary     \\"'
		`"1961 State	& Tribe 	& ST 	& m = 8 & ASJP & nodes 		& Tongue	& Lang.  & school (\%)	& Tribe 	& ST 	& m = 8 & ASJP & nodes 		& Tongue	& Lang.  & school (\%) & Tribe 	& ST 	& m = 8 & ASJP & nodes 		& Tongue	& Lang.  & school (\%) \\"'
		`"\midrule"')
		foot(`"\bottomrule"')
		;
	#delimit cr
