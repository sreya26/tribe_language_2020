...
*********************************************************************************************************************
*********************************************************************************************************************
* controlling for fraction of all STs that are Hindu and Christian
*********************************************************************************************************************	
...
*********************************************************************************************************************
* controlling for fraction of overall popn that are Hindu and Christian
*********************************************************************************************************************	
	...
	foreach area of local areas {
		...
		foreach dtype of local dist_types {
			foreach type of local match_to {
				...
				local basic_x lang_dist_normalized lang_dist_norm_sq  
				local dummies_x  i.castegroup_1961_2011_code i.region6113 
				local religion_x  hindu_frac_all_1961  christian_frac_all_1961  /*buddhist_frac_all_1961 jain_frac_all_1961 sikh_frac_all_1961 others_frac_all_1961 */ 
				label var wt_lang_dist_`dtype'_`type' "Language Distance"
				...
				#delimit ;
				esttab LITERACY_Y61 PRIMARYPLUS_Y61 MIDDLEPLUS_Y61 MATRICPLUS_Y61 GRADUATE_Y61
					using "../04_results/01_tables/12_regression_`type'_`dtype'_`area'_alltribes.tex", replace
					booktabs
					nobase nonumbers mtitles label b(3) se star(* 0.10 ** 0.05 *** 0.01) stats(N, fmt(%18.0gc) labels(`"Observations"') layout({@}))
					drop(*castegroup_1961_2011_code* *region6113*)
					nonotes
	...
	foreach area of local areas {
		...
		foreach dtype of local dist_types {
			foreach type of local match_to {
				...
				#delimit ;
				esttab LITERACY_Y61 PRIMARYPLUS_Y61 MIDDLEPLUS_Y61 MATRICPLUS_Y61 GRADUATE_Y61
					using "../04_results/01_tables/11_regression_`type'_`dtype'_`area'_big10.tex", replace
					booktabs
					nobase nonumbers mtitles label b(3) se star(* 0.10 ** 0.05 *** 0.01) stats(N, fmt(%18.0gc) labels(`"Observations"') layout({@}))
					drop(*castegroup_1961_2011_code* *region6113*)
					nonotes
	...
	foreach area of local areas {
		...
		foreach dtype of local dist_types {
			foreach type of local match_to {
				...
				#delimit ;
				esttab LITERACY_Y61 PRIMARYPLUS_Y61 MIDDLEPLUS_Y61 MATRICPLUS_Y61 GRADUATE_Y61
					using "../04_results/01_tables/12_regression_`type'_`dtype'_`area'_big10.tex", replace
					booktabs
					nobase nonumbers mtitles label b(3) se star(* 0.10 ** 0.05 *** 0.01) stats(N, fmt(%18.0gc) labels(`"Observations"') layout({@}))
					drop(*castegroup_1961_2011_code* *region6113*)
					nonotes
